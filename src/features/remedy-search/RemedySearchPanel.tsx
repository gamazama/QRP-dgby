...
const remedyImageUrl = (r: Remedy): string | null =>
  r.image ? `${import.meta.env.BASE_URL}packs/${r.packId}/${r.image.light}` : null;

export function RemedySearchPanel({ onAdd }: { onAdd: (r: Remedy) => void }) {
  const { remedies } = useRepositories();
  const [text, setText] = useState('');

  // Load the pack index + each pack's remedies once (lazy, cached by Query).
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mb-1 px-0.5 text-xs text-slate-500 dark:text-slate-400">
