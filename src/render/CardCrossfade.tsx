...
// fades out and is removed after `durationMs`. Inner spins keep running on the
// incoming card (it's a live render). durationMs <= 0 = instant cut.
export function CardCrossfade({
  ...
  useEffect(() => {
    ...
    if (k === prev.current.key) {
      prev.current = { key: k, node: children };
      return;
    }
    if (durationMs > 0) {
