  ...
  return (
    <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 md:px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm transition-colors duration-300">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">QRP Generator</h1>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
           ...
           <button
              onClick={onImport}
              aria-label="Load configuration from URL or JSON"
           ...
           >
              <Download size={18} />
              <span className="hidden sm:inline text-sm font-medium">Load Config</span>
           </button>

           {/* Save Button (JSON) */}
           <button
              onClick={onSave}
              aria-label="Save configuration as JSON"
              className="p-2 sm:px-3 rounded-md text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-2"
              title="Save Configuration as JSON"
           >
              <Save size={18} />
              <span className="hidden sm:inline text-sm font-medium">Save Config</span>
           </button>

           {/* Share Button (URL) */}
           <button
              onClick={onShare}
              aria-label="Share configuration by copying a link"
              className="p-2 sm:px-3 rounded-md text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-2"
              title="Share Configuration (Copy URL)"
           >
              <Link size={18} />
              <span className="hidden sm:inline text-sm font-medium">Share Link</span>
           </button>
